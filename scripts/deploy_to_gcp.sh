...
VM_DIR="${GCP_VM_DIR:-/home/jyx/lark-bot-deploy/lark-bot-gcp}"
...
restart_and_verify() {
    ...
    $ssh_cmd "sudo systemctl restart lark-bot"
    sleep 3
    ...
    health=$($ssh_cmd "curl -sf http://localhost:7777/health 2>&1") || {
        ...
        $ssh_cmd "sudo journalctl -u lark-bot -n 30 --no-pager" || true
        echo ""
        echo "请登录 VM 手动排查："
        echo "  sudo systemctl status lark-bot"
        echo "  sudo journalctl -u lark-bot -n 50"
        return 1
    }
